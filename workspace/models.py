...
class NearEarthObject:
    ...
    def __init__(self, designation, hazardous, name='None', diameter=float('nan')):
        ...
        :param name: The IAU name for this `NearEarthObject`, it is not mandatory, if not supplied, defaults to `None`.
        :param diameter: The diameter, in kilometers, of this `NearEarthObject`, it is not mandatory, if not supplied, defaults to `nan`.
        """
    ...
    def fullname(self):
        ...
        fName = self.designation if self.name is None else f"{self.designation} ({self.name})"
        return fName
    ...
    def __str__(self):
        ...
        return f"NEO {self.fullname} has a diameter of {self.diameter:.3f} km and is {'' if self.hazardous else 'not '}potentially hazardous."
...
class CloseApproach:
    ...
    def __init__(self, designation, time, distance, velocity):
        ...
        try:
            self.time = cd_to_datetime(time)  # DONE: Use the cd_to_datetime function for this attribute.
            self.distance = float(distance)
            self.velocity = float(velocity)
        except ValueError:
            print("Bad arguments provided for constructor of CloseApproach")
    ...
    def __str__(self):
        ...
        # The project instructions include one possibility. Peek at the __repr__
        # method for examples of advanced string formatting.
        return f"On {self.time_str}, '{self.neo.fullname if self.neo is not None else self._designation}' approaches Earth at a distance of {self.distance:.2f} au and a velocity of {self.velocity:.2f} km/s."
